...
(function() {
  ...
  window.autoTagAllLocationsUnified = async function(options = {}) {
    ...
    try {
      ...
      if (showProgress) {
        progressContainer = createProgressContainer(adventuresData.length);
      }
      ...
      for (let index = 0; index < adventuresData.length; index++) {
        ...
        try {
          ...
          const recommendations = getTagRecommendationsForLocation(location, tagManager);
   ...
   */
  function getTagRecommendationsForLocation(location, tagManager) {
    ...
    try {
      ...
      if (tagManager) {
        tagManager.currentPlaceId = placeId;

        // Build recommendations using the same engine as tag button
        const recs = tagManager.buildRecommendations();

        console.log(`📝 Got ${recs.length} recommendations for ${values[0]}`);
        return recs || [];
      }
